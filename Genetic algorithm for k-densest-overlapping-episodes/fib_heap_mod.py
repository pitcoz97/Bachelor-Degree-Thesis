...
#Added: getter methods for key and value in Node class,
#       __bool__ method for the FibonacciHeap class,
#       comparison with <= instead of just < nell'ultimo ciclo della consolidate,
#       risetto: self.root_list = self.min_node alla fine della consolidate()
#       the size of A[d] is set to: A = [None] * int(math.log(self.total_nodes) * 2.08 + 1)
    ...
    def extract_min(self):
        ...
        if z is not None:
            if z.child is not None:
                #print('parent %d' % z.get_value())
                # attach child nodes to root list
                #print('figlio %d' % z.child.get_value())
